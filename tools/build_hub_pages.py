...
# --- accessibility type scale, 2026-09-12 -----------------------------------
# Colin raised the font size on the 10 Sep call; Romil confirmed on 12 Sep that
# it is "the only piece left". The hub is a 1920 canvas on FitToPage, so the
# Service scales it to ~0.59 in a normal window - 9px authored text lands at
# 5.3px on screen. The canvas is what makes the type small (the four Gold
# reports sit on 1280 and read fine at 12-16px), so the fix is a bigger
# authored size, not a zoom setting. TB_LINE moves 21 -> 30 to match.
TYPE = {8: 13, 9: 15, 10: 16, 11: 17, 12: 19, 13: 21, 14: 22, 16: 24, 17: 26}
CARD_TYPE = {8: 10, 9: 11, 10: 12, 11: 13}          # cardVisual sizes are POINTS


def ts(px):
    """Scale an authored textbox size. Sizes already above the body range
    (26/30/34 - the big KPI numerals) are left alone."""
    return TYPE.get(px, px)


def cs(pt):
    """Scale a cardVisual point size."""
    return CARD_TYPE.get(pt, pt)


# Every manual height calculation in this file passes the AUTHORED size, so wrap
# the two metrics helpers to scale first. Without this the boxes stay sized for
# the old type and everything overflows its tile.
_tb_height_raw, _card_height_raw = tb_height, card_height


def tb_height(txt, box_w, font_px, bold=False, lines=None):
    return _tb_height_raw(txt, box_w, ts(font_px), bold, lines)


def card_height(size_pt=10, pill=False):
    return _card_height_raw(cs(size_pt), pill)


# --- element helpers ---------------------------------------------------------
def vcy(y, h, px=None):
    """Top for a one-line textbox whose glyph should centre on a y..y+h shape.
    The line box sits TB_VPAD/2 below the box top."""
    line = TB_LINE if px is None else max(TB_LINE, int(round(px * 1.35)))
    return y + (h - line) // 2 - TB_VPAD // 2


def text(x, y, w, runs, align="left", lines=None, bg=None):
    """Auto-heights to the Service's 21px line box. Returns the height used."""
    runs = [(v, ts(sz), c, bd) for v, sz, c, bd in runs]
...
def report_cards(cards, top=None):
    ...
    q_w = cw - pad * 2
    # wrap against the SCALED size - measuring at the authored size under-counts
    # lines and the Service then clips the overflow silently.
    q_lines = max(wrap_lines(c[5], q_w - TB_HPAD, ts(10)) for c in cards)
    t_lines = max(wrap_lines(c[0], title_w - TB_HPAD, ts(title_size), True)
                  for c in cards)
    th = TB_LINE * t_lines + TB_VPAD
    qh = TB_LINE * q_lines + TB_VPAD
    upd_h = card_height(9)
    head_h = max(th, card_height(9))
    # padding trimmed 22->14 on 2026-09-12: the larger question type takes a
    # second line, and the bottom panels need the room back.
    ch = 14 + head_h + 8 + (TB_LINE + TB_VPAD) + 2 + qh + 10 + 1 + 8 + upd_h + 14
...
def commentary_panel(heading, subtitle, rows, y, h):
    ...
    for label, colour, m in rows:
        ...
        # these labels are allowed to wrap now - forcing lines=1 clipped the
        # longer window names in the Service, which the bounds check cannot see
        l_lines = wrap_lines(label, label_w - 14 - TB_HPAD, ts(10), True)
...
def contacts_panel(contacts, links, y, h):
    ...
    # Row pitches are DERIVED, not fixed: at the 2026-09-12 type scale a name is
    # 42px tall, so the old hard-coded +26 / +28 sub offsets collided.
    for initials, name, sub in contacts:
        oval(x + 24, cy + 2, 34, 34, NAVY)
        text(x + 24, vcy(cy + 2, 34, ts(9)), 34, [(initials, 9, WHITE, True)],
             align="center")
        nh = text(x + 70, cy, w - 94, [(name, 10, INK, True)], lines=1)
        sh = text(x + 70, cy + nh - 8, w - 94, [(sub, 9, NAVY, False)], lines=1)
        cy += nh - 8 + sh + 8
    rect(x + 24, cy + 2, w - 48, 1, fill=LINE, radius=0)
    cy += 14
    btn_h = ts(13) + 18
    for label, sub, target in links:
        rect(x + 24, cy + (btn_h - 14) // 2, 14, 14, fill=NAVY, radius=2)
        button(x + 46, cy, w - 94, btn_h, label, color=NAVY, size=9,
               align="left", link_to=target)
        cy += btn_h + 10
    _regions.pop()
    return y + h
...
HELP_LINKS_OPS = [("Definitions and sources",
...
col_w = CONTENT_W // 4
desc_lines = max(wrap_lines(d, col_w - 34 - TB_HPAD, ts(9)) for _, _, d in BADGE_DOC)
...
TOP_Y = y + 16
step_w = COMM_W - 48 - 46
step_lines = max(wrap_lines(d, step_w - TB_HPAD, ts(9)) for _, _, d in STEPS)
...
ST_Y = TOP_Y + TOP_H + 16
st_col = CONTENT_W // 3
st_lines = max(wrap_lines(d, st_col - 34 - TB_HPAD, ts(9)) for _, _, d in STATES)
...
button(W // 2 - 240, ay, 230, 36, "Request access", color=WHITE, size=10, fill=NAVY,
       border=NAVY)
button(W // 2 + 10, ay, 230, 36, "Who should I ask?", color=NAVY, size=10, border=LINE)
ay += 36 + 12
