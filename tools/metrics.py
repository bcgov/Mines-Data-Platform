...
# --- textbox -----------------------------------------------------------------
TB_LINE = 30          # paragraph line box. Was 21, which suited the 8-13px
                      # type this hub launched with. Raised 2026-09-12 for the
                      # accessibility pass (Colin/Romil): at 14-24px the Service
                      # line box grows with the font, verified live on Home.
...
# --- card sizing -------------------------------------------------------------
# Measured on the live report: 41px at 9pt, 49px at 13pt - so it DOES creep up
# with the point size, ~2px per point. The old code pinned it at a flat 48 and
# capped every card at 11pt to stay under that. Now it is derived, which lets
# the accessibility pass use 12-14pt cards without over-reserving on small ones.
CARD_CONTENT_H = 48   # kept for callers that want the old flat value


def card_line_h(size_pt=10):
    """Height the value block occupies inside the card's content area."""
    return int(math.ceil(25 + 2.0 * size_pt))
...
def fit_size(sample, box_w, pill=False, bold=False, sizes=(13, 12, 11)):
