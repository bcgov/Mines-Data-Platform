resource "azurerm_key_vault_secret" "jumpboxadminuser" {
  ...
  depends_on      = [azurerm_key_vault.kv, azurerm_private_endpoint.kv] //, azurerm_role_assignment.sp_keyvault_secret_reader]
...
resource "azurerm_key_vault_secret" "jumpboxadminpassword" {
  ...
  key_vault_id    = azurerm_key_vault.kv.id
  depends_on      = [azurerm_key_vault.kv, azurerm_private_endpoint.kv]
  expiration_date = timeadd(timestamp(), "2160h")
}